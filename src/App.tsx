...
export default function App() {
  ...
  const [characterState, setCharacterState] = useState<"idle" | "listening" | "talking">("idle");
  ...
  useEffect(() => {
    if (state === "speaking") {
      if (activeEmotion === "playful") {
        setCharacterState("giggling");
      } else {
  ...
  useEffect(() => {
    sessionRef.current = new MyraaAudioSession({
      ...
      onTranscription: (role, text, overwrite, emotion) => {
        if (role === "user") {
          setUserCaption(text);
          ...
          if (overwrite) {
            ...
            if (emotion) {
              setActiveEmotion(emotion as any);
            } else {
              const newEmotion = detectEmotionFromText(text);
  ...
  const handleToggleConnection = async () => {
    ...
    if (state === "disconnected") {
      await sessionRef.current.connect(selectedVoice);
    } else {
      sessionRef.current.disconnect();
    }
  };
  ...
  return (
    <div
      ...
      className={`relative w-full h-[100dvh] overflow-hidden bg-[#020205] text-white ${getAmbientStyles()} theme-transition flex flex-col justify-between p-4 sm:p-10 select-none pb-8 sm:pb-10`}
    >
      ...
      <header className="relative z-30 flex items-center justify-between w-full max-w-5xl mx-auto select-none">
        ...
        <div className="flex items-center gap-5">
          ...
          <select 
            ...
            className="bg-transparent text-white/50 hover:text-white transition text-xs font-mono tracking-widest cursor-pointer outline-none border-none appearance-none disabled:opacity-30"
            title="Switch Voice Model (Must be disconnected)"
          >
            <option className="bg-slate-900" value="Leda">VOICE: LEDA</option>
            <option className="bg-slate-900" value="Aoede">VOICE: AOEDE</option>
            <option className="bg-slate-900" value="Kore">VOICE: KORE</option>
            <option className="bg-slate-900" value="Puck">VOICE: PUCK</option>
            <option className="bg-slate-900" value="Charon">VOICE: CHARON</option>
            <option className="bg-slate-900" value="Fenrir">VOICE: FENRIR</option>
          </select>
          ...
          {showGuide && (
            ...
            >
              ...
              <p className="text-xs text-slate-400 mb-4 font-mono leading-relaxed">
                Ohana is equipped with dynamic visual modules and standard text browser projectors. Here are clever triggers to try speaking aloud:
              </p>
              <div className="space-y-2 text-xs font-serif italic text-indigo-300">
                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition cursor-pointer font-sans normal-case text-slate-200">
                  ⚡ &quot;Ohana, change atmosphere of your core to crimson&quot; <span className="text-[10px] font-mono text-indigo-400 block mt-0.5 font-medium">Shifts theme color background</span>
                </div>
                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition cursor-pointer font-sans normal-case text-slate-200">
      ...
      <footer className="relative z-10 w-full max-w-2xl mx-auto flex flex-col items-center gap-5 mt-auto">
        ...
        <div className="flex items-center justify-center relative mb-4">
          <button 
            ...
            title={state === "disconnected" ? "Awake Ohana" : "Sleep core"}
          >
            {state === "disconnected" ? (
