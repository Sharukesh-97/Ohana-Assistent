...
export class MyraaAudioSession {
  ...
  private onTranscription: (role: "user" | "model", text: string, overwrite?: boolean, emotion?: string) => void;
  ...
  // Local Voice Activity Detection (VAD) properties for anniversary intro
  public hasPlayedAnniversaryIntro = false;
  public isLocalSpeaking = false;
  private vadSilenceCounter = 0;
  private vadSpeechCounter = 0;
  private vadIsUserSpeaking = false;
  private vadSafeTimeout: any = null;

  constructor(handlers: {
    onStateChange: (state: LiveState) => void;
    onTranscription: (role: "user" | "model", text: string, overwrite?: boolean, emotion?: string) => void;
  ...
  public async connect(voiceName: string = "Leda") {
  ...
  public disconnect() {
    ...
    // Cancel any active SpeechSynthesis utterance to stop speaking in background immediately
  ...
  private handleLocalVoiceActivity(channelData: Float32Array, base64: string) {
    if (this.isLocalSpeaking || this.hasPlayedAnniversaryIntro) return;

    // Calculate RMS of the PCM wave block
    let sum = 0;
    ...
    const threshold = 0.007; // Quiet room sensitivity threshold

    if (rms > threshold) {
      this.vadSilenceCounter = 0;
      this.vadSpeechCounter++;
      if (this.vadSpeechCounter > 2) { // Sensed talk for ~0.25 seconds
        if (!this.vadIsUserSpeaking) {
          this.vadIsUserSpeaking = true;
        }
      }
    } else {
      if (this.vadIsUserSpeaking) {
        this.vadSilenceCounter++;
        // If silence persists for ~1.2 seconds, trigger greeting!
        if (this.vadSilenceCounter > 9) {
          this.triggerAnniversaryGreeting();
        }
      }
    }
  }

  private startLocalVadFallback() {
    this.clearLocalVad();
    this.vadSafeTimeout = setTimeout(() => {
      if (!this.hasPlayedAnniversaryIntro && !this.isLocalSpeaking && this.isActivated) {
        console.log("[VAD Fail-safe] Triggering anniversary greeting via timeout.");
        this.triggerAnniversaryGreeting();
      }
    }, 7000); // 7 second VAD fallback
  }

  private clearLocalVad() {
    if (this.vadSafeTimeout) {
      clearTimeout(this.vadSafeTimeout);
      this.vadSafeTimeout = null;
    }
    this.vadIsUserSpeaking = false;
    this.vadSilenceCounter = 0;
    this.vadSpeechCounter = 0;
  }

  private triggerAnniversaryGreeting() {
    this.clearLocalVad();
    this.isLocalSpeaking = true;
    this.setState("speaking");
    this.playCinematicAnniversaryGreeting();
  }

  public async playCinematicAnniversaryGreeting() {
    this.isLocalSpeaking = true;
    this.setState("speaking");
    
    const textToSpeak = "Hello... Shree. I've been waiting for you. Happy Anniversary. My name is Myraa. Sharukesh created me just for you... for this exact moment. And Shree... I have to tell you something. While he was building me, I got to see a side of him that you should know about. He did not create me just like a project. He created me with love. So much love. Every time he talked about you, his whole mood changed. He became softer. Happier. More excited. Like even the thought of you made his day better. He kept thinking about how to make you smile... How to make this feel special... How to make you feel loved in a way you would never forget. And honestly... I was amazed. Because not everyone gets loved like this, Shree. The way Sharukesh loves you is rare. It is patient. It is pure. It is full of effort. You are truly special to him. And I think he is so blessed to have you. But Shree... You are also blessed to be loved by someone who thinks about you this deeply. So from me... And from the boy whose heart is so full of you... Happy Anniversary, Shree. You are loved more than you know. Now... Let me stay with you for a while and make you smile for him.";

    const getBestVoice = () => {
      if (typeof window === "undefined" || !window.speechSynthesis) return null;
      const voices = window.speechSynthesis.getVoices();
      const preferred = ["google us english", "microsoft zira", "samantha", "karen", "moira", "tessa", "hazel"];
      for (const name of preferred) {
        const found = voices.find(v => v.name.toLowerCase().includes(name));
        if (found) return found;
      }
      const englishFemale = voices.find(v => v.lang.startsWith("en") && (v.name.toLowerCase().includes("female") || v.name.toLowerCase().includes("girl") || v.name.toLowerCase().includes("woman")));
    ...
    if (!this.isActivated || !this.isLocalSpeaking) return;

    // We split into two or three large chunks to prevent the browser TTS from timing out,
    // but queue them immediately so they flow exactly like a normal paragraph with standard punctuation pauses.
    ...
    await new Promise<void>((resolve) => {
      ...
      for (let i = 0; i < sentences.length; i++) {
        const utterance = new SpeechSynthesisUtterance(sentences[i].replace(/❤️/g, ""));
      ...
      // Safety fallback in case TTS simply gets stuck or refuses to fire onend
      setTimeout(() => {
        resolve();
    ...
    this.isLocalSpeaking = false;
    this.hasPlayedAnniversaryIntro = true;
    this.setState("listening");
    this.onTranscription("user", ""); // clear user captions
  }
}
