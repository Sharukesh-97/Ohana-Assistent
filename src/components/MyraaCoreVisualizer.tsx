...
interface MyraaCoreVisualizerProps {
  ...
  characterState: "idle" | "listening" | "talking";
}
...
export const MyraaCoreVisualizer: React.FC<MyraaCoreVisualizerProps> = ({
  ...
  useEffect(() => {
    ...
    if (characterState === "idle") {
      ...
      pauseVideo(listeningVideoRef.current);
      pauseVideo(talkingVideoRef.current);
    } else if (characterState === "listening" || characterState === "thinking" as any) {
      playVideo(listeningVideoRef.current);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (state === "speaking" && (session?.outputAnalyser || (session as any)?.isLocalSpeaking)) {
        if (session?.outputAnalyser) {
          activeAnalyser = session.outputAnalyser;
        } else {
          // Speech synthesis mock amplitude: oscillation with slight pseudo-random jitter for realistic lip sync and particle reactions
          const offsetTime = systemTime * 0.012;
          const osc = Math.sin(offsetTime) * 45 + Math.cos(offsetTime * 0.5) * 30 + 95;
          const jitter = (0.4 + Math.sin(systemTime * 0.09) * 0.4) * osc;
          audioLevel = Math.max(15, Math.min(170, jitter));
        }
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      >
        <div className="relative w-full max-w-4xl h-[50dvh] sm:h-auto sm:aspect-[16/9] flex items-center justify-center sm:scale-110 select-none pointer-events-none md:max-h-[72vh] -mt-10 sm:mt-0 drop-shadow-2xl">
          {/* Subtle Outer Ambient Shadow Cast */}
          <div className="absolute inset-0 rounded-[2.5rem] blur-[30px] opacity-30 bg-cyan-600/20 pointer-events-none mix-blend-screen" />
          {/* Mobile premium glass border */}
          <div className="absolute inset-0 rounded-[2.5rem] border border-white/10 bg-black/10 sm:hidden pointer-events-none z-10 shadow-[0_0_40px_rgba(34,211,238,0.15)] ring-1 ring-white/5" />
          ...
          <video
            ...
            src="/assets/listening.mp4"
            loop
            muted
            playsInline
            className={`absolute inset-0 w-full h-full object-cover rounded-[2.5rem] transition-opacity duration-700 ease-in-out ${
              characterState === "listening" || (characterState as any) === "thinking" ? "opacity-100 z-10 animate-fade-in" : "opacity-0 z-0"
            }`}
            style={{
              maskImage: "radial-gradient(circle, rgba(0,0,0,1) 55%, rgba(0,0,0,0) 80%)",
              WebkitMaskImage: "radial-gradient(circle, rgba(0,0,0,1) 55%, rgba(0,0,0,0) 80%)",
